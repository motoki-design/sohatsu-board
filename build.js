 ...
 *
 *   SOHATSU_PASS='合言葉' node build.js
 *
 * 暗号：PBKDF2-SHA256（250,000回）で鍵を導出し AES-256-GCM。
 * 生成される index.html には暗号文しか入らないので、GitHubで公開しても
 * 合言葉を知らない人には案件データもシートIDも読めない。
 *
 * ★2026-09-07 新設（結 0905便 §2-2）。toku-board の build.js をそのまま横へ広げた。
 *   同じ問題に2つ目の解を作らない。
 * ★案件マスタ（GSheet）の共有を閉じるのは守山さん（段3）。それが済むまでは、
 *   マスタのURLを別経路で知っている人はシートを直接見られる。
 */
const fs = require('fs');
...
const ITER = 250000;
const PAYLOAD = process.env.SOHATSU_PAYLOAD
  || (function () {
    try {
      return JSON.parse(fs.readFileSync(path.join(__dirname, 'config.local.json'), 'utf8')).payloadPath;
    } catch (e) {
      console.error('[build] SOHATSU_PAYLOAD が未設定です。環境変数か config.local.json の "payloadPath" を使ってください。');
      process.exit(2);
    }
  })();
...
const pass = process.env.SOHATSU_PASS || (function () {
  try {
...
console.log(`  案件 ${(data.projects?.rows || []).length} 件（マスタ取得 ${data.projects?.as_of ?? '—'} 時点）`);
if (!data.projects?.rows?.length) {
  console.error('★案件が0件です。refresh.js が失敗していないか確かめてください');
  process.exitCode = 1;
}
